...
router.post('/profile', authMiddleware, upload.single('avatar'), async function (req, res, next) {
  try {
    ...
    const isGeminiBusy = error?.status === 503 || String(error?.message || '').includes('"status":"UNAVAILABLE"');
    const statusCode = isGeminiBusy ? 503 : 500;
    const userMessage = isGeminiBusy
      ? 'Gemini is temporarily overloaded. Please retry in a minute.'
